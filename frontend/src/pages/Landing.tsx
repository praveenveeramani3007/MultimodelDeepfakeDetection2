  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-hidden">
      ...
      <nav className="fixed w-full z-50 bg-background/80 backdrop-blur-md border-b border-border/50">
        <div className="max-w-7xl mx-auto px-4 xxs:px-6 h-16 flex items-center justify-between">
      ...
      {/* Hero Section */}
      <section className="relative pt-24 xxs:pt-32 pb-16 xxs:pb-20 lg:pt-48 lg:pb-32 px-4 xxs:px-6">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            ...
            <h1 className="text-3xl xxs:text-4xl md:text-5xl lg:text-7xl font-display font-bold leading-tight mb-6 xxs:mb-8">
              Verify Truth in a <br />
              <span className="text-gradient-primary">Digital World</span>
            </h1>
            <p className="text-base xxs:text-lg md:text-xl text-muted-foreground mb-8 xxs:mb-10 max-w-2xl mx-auto leading-relaxed px-2 xxs:px-4">
              Detect deepfakes, analyze sentiment, and verify content authenticity with our advanced multimodal forensic dashboard.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 w-full sm:w-auto px-2 xxs:px-6">
              <a href="/api/login" className="w-full sm:w-auto">
                <button className="w-full sm:w-auto px-6 xxs:px-8 py-3 xxs:py-4 rounded-xl bg-primary text-white font-bold text-base xxs:text-lg hover:bg-primary/90 shadow-lg shadow-primary/25 transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2">
                  Get Started <ArrowRight className="w-5 h-5" />
                </button>
              </a>
              <button className="w-full sm:w-auto px-8 py-4 rounded-xl bg-secondary text-foreground font-bold text-lg hover:bg-secondary/80 border border-border transition-all">
                View Demo
              </button>
            </div>
          </motion.div>
      ...
      {/* Features Grid */}
      <section className="py-16 xxs:py-20 bg-secondary/20 border-t border-border/50">
        <div className="max-w-7xl mx-auto px-4 xxs:px-6">
...
// Icon Wrapper to handle Lucide props cleanly
const FeatureIcon = ({ icon: Icon }: { icon: any }) => <Icon className="w-6 h-6" />;
